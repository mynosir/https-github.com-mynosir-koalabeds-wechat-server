$(function() {
            ...
            var callback = function(res) {
                if(res.status == 0) {
                    var list = res['data']['list'],
                        listTpl = '<tr><th>Serial No.</th><th>Name</th><th>Menu Authorization</th><th>Create User</th><th>Create Time</th><th>Update User</th><th>Update Time</th><th>Operation</th></tr>';
                    for(var i in list) {
                        listTpl += '<tr>';
    ...
    $('body').delegate('.js_add_saveBtn', 'click', function() {
        var name = $('.js_add_name').val(),
            pms = '';
        $('input:checkbox[name=pms]:checked').each(function(i) {
            if(0 == i) {
                pms = $(this).val();
            } else {
                pms += (',' + $(this).val());
