        ...
        init: function(p) {
            ...
            var callback = function(res) {
                ...
                var list = res['list'],
                    ...
                    listTpl = '<tr><th>no.</th><th>openid</th><th>酒店id</th><th>客户名称</th><th>客户邮箱</th><th>客户列表</th><th>预定id</th><th>订单生成时间</th><th>订单信息修改时间</th><th>到店时间，24小时制</th><th>订单来源</th><th>订单状态</th><th>总价格</th><th>余款</th><th>余款细节</th><th>分配的酒店房间详情</th><th>未分配的酒店房间详情</th><th>信用卡信息</th><th>入住日期</th><th>退房日期</th></tr>';
                for(var i in list) {
                    listTpl += '<tr>';
                    listTpl += '<td>' + list[i]['id'] + '</td>';
                    listTpl += '<td>' + list[i]['openid'] + '</td>';
                    listTpl += '<td>' + list[i]['propertyID'] + '</td>';
                    listTpl += '<td>' + list[i]['guestName'] + '</td>';
                    listTpl += '<td>' + list[i]['guestEmail'] + '</td>';
                    listTpl += '<td>' + list[i]['guestList'] + '</td>';
                    listTpl += '<td>' + list[i]['reservationID'] + '</td>';
                    listTpl += '<td>' + list[i]['dateCreated'] + '</td>';
                    listTpl += '<td>' + list[i]['dateModified'] + '</td>';
                    listTpl += '<td>' + list[i]['estimatedArrivalTime'] + '</td>';
                    listTpl += '<td>' + list[i]['source'] + '</td>';
