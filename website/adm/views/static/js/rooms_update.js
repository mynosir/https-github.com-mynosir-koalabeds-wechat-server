...
$(function() {
    var page = {
        init: function() {
          $('#roomTypeDescription').summernote({
              toolbar: [
                  <!--字体工具-->
            ...
            $('#roomTypeDescription_cn').summernote({
                toolbar: [
                    <!--字体工具-->
                    ['fontname', ['fontname']], //字体系列
                    ['style', ['bold', 'italic', 'underline', 'clear']], // 字体粗体、字体斜体、字体下划线、字体格式清除
    ...
    $('body').delegate('.js_submit', 'click', function() {
        ...
        var roomTypeID = $('#roomTypeID').val(),
            ...
            roomTypeName_cn = $('#roomTypeName_cn').val(),
            roomTypeNameShort_cn = $('#roomTypeNameShort_cn').val(),
            roomTypeDescription_cn = $('#roomTypeDescription_cn').summernote('code');
        var id = $('.js_id').val();
        var json = {
            api: config.apiServer + 'rooms/post',
            type: 'post',
            data: {
                actionxm: 'save',
                id: id,
                params: {
                    ...
                    roomTypeName: roomTypeName_cn||'',
                    roomTypeNameShort: roomTypeNameShort_cn||'',
                    roomTypeDescription: roomTypeDescription_cn||''
                }
            }
        };
        var callback = function(res) {
            if(res.status == 0) {
                alert('Save success!');
                window.location.href = "/adm/rooms";
            } else {
                alert(res.msg);
            }
        };
