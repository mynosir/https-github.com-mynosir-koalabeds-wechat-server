...
create table `ko_user` (
    ...
    `wx_subscribe` int(2) default 0 comment '是否关注',
    ...
    `wx_headimgurl` varchar(512) default '' comment '微信头像',
    `wx_subscribe_time` int default 0 comment '微信关注时间',
    `wx_remark` varchar(512) default '' comment '微信备注',
    `wx_groupid` int default 0 comment '微信分组ID',
    `wx_tagid_list` text comment '微信标签ID列表',
